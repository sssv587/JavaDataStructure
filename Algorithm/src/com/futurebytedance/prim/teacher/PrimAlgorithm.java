package com.futurebytedance.prim.teacher;
...
public class PrimAlgorithm {
    public static void main(String[] args) {
        ...
        //邻接矩阵的关系用二维数组表示,10000这个大数，表示两个点不连通
        int[][] weight = new int[][]{
...
//        }
        ...
        for (int k = 1; k < graph.vertx; k++) { //因为有graph.vertx顶点，普利姆算法结束后，有graph.vertx-1边
            //这个是确定每一次生成的子图，和哪个节点的距离最近
            for (int i = 0; i < graph.vertx; i++) { //i节点表示被访问过的节点
                for (int j = 0; j < graph.vertx; j++) {//j节点表示还没有访问过的节点
